...
export function renderSubmissionTakedownEmail(
  ...
  const copy =
    current === "es"
      ? {
          subject: `Tu mascota fue retirada de AgentPets: ${vars.petName}`,
          intro: `Hola, tu mascota "${vars.petName}" fue retirada de AgentPets y sus archivos fueron eliminados.`,
          noReason:
      ...
      : current === "zh"
        ? {
            subject: `你的宠物已从 AgentPets 下架：${vars.petName}`, // fixme:zh
            intro: `你好，你的宠物“${vars.petName}”已从 AgentPets 下架，相关文件也已删除。`, // fixme:zh
            noReason: "没有提供公开原因。该 slug 已释放，你可以重新提交。", // fixme:zh
            cta: `如需上传新版本，请前往：${submitUrl}`, // fixme:zh
          }
        : {
            subject: `Your pet was taken down from AgentPets: ${vars.petName}`,
            intro: `Hey, your pet "${vars.petName}" was taken down from AgentPets and its files were removed.`,
            noReason:
