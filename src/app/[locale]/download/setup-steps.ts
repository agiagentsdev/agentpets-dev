 ...
 *   2. `npx -y https://github.com/agiagentsdev/agentpets-dev/releases/latest/download/agentpets-cli.tgz init`   <- wires hooks + wakes the mascot
 *   *. `npx -y https://github.com/agiagentsdev/agentpets-dev/releases/latest/download/agentpets-cli.tgz install <slug>` (optional, when /pets/<slug>
 *       sent the user here with ?next=install/<slug>)
 *   *. `npx -y https://github.com/agiagentsdev/agentpets-dev/releases/latest/download/agentpets-cli.tgz update` (dimmed, runs anytime)
 *
  ...
  const steps: SetupStep[] = [
    {
      ...
      command: "npx -y https://github.com/agiagentsdev/agentpets-dev/releases/latest/download/agentpets-cli.tgz init",
      hint: t("setup.step1.hint"),
    },
  ];

  if (pendingInstallSlugs && pendingInstallSlugs.length > 0) {
    const installCommand = `npx -y https://github.com/agiagentsdev/agentpets-dev/releases/latest/download/agentpets-cli.tgz install ${pendingInstallSlugs.join(" ")}`;
    steps.push({
      key: "installPet",
      title:
