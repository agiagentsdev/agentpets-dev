...
<p>
  ...
  &nbsp;·&nbsp;
  <a href="https://agentpets.dev/built-with">Built with Petdex</a>
  &nbsp;·&nbsp;
...
<p>
  ...
  <a href="https://github.com/agiagentsdev/agentpets-dev/issues"><img src="https://img.shields.io/github/issues/agiagentsdev/agentpets-dev?style=flat-square&color=000000" alt="GitHub issues" /></a>
</p>
...
1. **A web gallery** at [agentpets.dev](https://agentpets.dev) where the community submits, reviews, and showcases animated pets in the Codex sprite format.
...
Every pet is a folder. Every folder is a Pokédex entry. Every entry is one `npx @agentpets/cli install` away.
...
| You want to... | Do this |
| --- | --- |
| Browse pets | Visit [agentpets.dev](https://agentpets.dev) |
| Install a pet | `npx @agentpets/cli install <slug>` |
| Run the desktop floater | `npx @agentpets/cli init` (downloads the `.dmg` and wires Codex/Claude Code hooks) |
| Make a pet | Use the `hatch-pet` skill inside Codex, or build one with the [Petdex creator tools](https://agentpets.dev/create) |
| Submit a pet | `npx @agentpets/cli submit ./my-pet/` or drop it through the web submitter |
...
- **The HTTP API.** `agentpets.dev/api/manifest` returns every approved pet with its slug, spritesheet URL, animation states, and metadata.
- **The pet package format.** Every pet is a `pet.json` plus a `spritesheet.{webp,png}` rendered as an 8×9 grid of 192×208 frames.

13 open-source projects already build on these. See [agentpets.dev/built-with](https://agentpets.dev/built-with) for the catalog, then [submit yours via the issue template](https://github.com/agiagentsdev/agentpets-dev/issues/new?template=built-with.yml).
...
- **Submit a pet:** [agentpets.dev/submit](https://agentpets.dev/submit) or `npx @agentpets/cli submit <path>`.
- **List your project:** open a [Built with Petdex issue](https://github.com/agiagentsdev/agentpets-dev/issues/new?template=built-with.yml).
...
Pets are user-submitted fan art. Petdex does not claim rights to any underlying IP. If you hold rights to a character and want a pet removed, file a [takedown request](https://github.com/agiagentsdev/agentpets-dev/issues/new?template=takedown.yml) and we review within 48 hours.
